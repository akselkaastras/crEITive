...
typedef boost::multi_array<double,3> Array3Double;
typedef boost::multi_array<dcomplex,3> Array3Complex;

//typedef boostublas::permutation_matrix<std::size_t> PermMat;

typedef boostublas::compressed_matrix<double,boostublas::column_major> CompMatDoubleColMaj;
...
inline double realfun(const dcomplex &z)
{
  return z.real();
}
...
inline boost::multi_array<double,ndims> realfunarray(const boost::multi_array<dcomplex,ndims> &A)
{
  VecUnsigned vshape (ndims);
  std::copy(A.shape(),A.shape()+ndims,vshape.begin());
  boost::multi_array<double,ndims> Ar(vshape);
  std::transform(A.origin(),A.origin()+A.num_elements(),Ar.origin(),realfun);
  return Ar;
};
