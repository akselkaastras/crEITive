...
void whittaker_shannon_calderon(VecComplex &sig,MatDouble &x,MatDouble &xi,Array3Complex &qhat)
{
  ...
  sig = -(VecComplex)boostublas::prec_prod(boostublas::element_div(qhatline,normxi2),complexfunmat(MatOp(xix,[](double c) -> double { return cos(c); }),MatOp(xix,[](double c) -> double { return sin(c); })))/(4.0*square(Pi));
  AutoVecOp(sig,plus_one<dcomplex>);
}
