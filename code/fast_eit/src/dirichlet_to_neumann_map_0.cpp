...
#pragma omp parallel default(none) shared(dnmap,lf,cp,wt,n,tn2,vn2)
  {
	...
	for (unsigned i=0;i<tn2;i++) //Row of dnmap
	  {
	    ...
	    for (unsigned nm=1;nm<vn2;nm++) //Loop on degrees and orders of spherical harmonics
	      {
		unsigned nn=(intsqrt(1+8*nm)-1)/2; //Degree of spherical harmonic
		unsigned mm=nm-(nn*(nn+1))/2; //Order of spherical harmonic
