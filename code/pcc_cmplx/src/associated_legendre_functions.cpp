...
void associated_legendre_functions(MatDouble &lf,VecDouble &t,unsigned &n)
{
  //std::cout << "Computing fully normalized associated Legendre functions..." << std::endl;

  //For manipulation of begenning and ending columns for a degree
  unsigned colbegpp;
  unsigned colbegp;
  unsigned colendp;
  unsigned colbeg;
  ...
  for (unsigned i=0;i<t.size();i++) //Fill the first three columns
    {
      lf(i,0)=invroot4Pi;
      lf(i,1)=sqrt(3.0)*t(i)*lf(i,0);
      lf(i,2)=sqrt(3.0/2.0)*sqrt(1.0-pow(t(i),2))*lf(i,0);
    }

  for (unsigned i=0;i<t.size();i++) //Values of t
    {
      colbegpp=0;
      colbegp=1;
      colendp=2;
      ...
      for (unsigned k=2;k<=n;k++) //Degrees of Legendre functions
	{
	  ...
	  lf(i,colend)=sqrt((2.0*(double)k+1.0)/(2.0*(double)k))*sqrt(1.0-pow(t(i),2))*lf(i,colendp);
