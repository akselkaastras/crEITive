		  ...
		  if (dofntj!=nnodes)
		    {
		      ...
		      if (dofnti==dofntj)
			{
			  M(dofnti,dofntj)+=(q(nt)/10.0+(double)boostublas::prec_inner_prod(graddofnti,graddofntj))*tetrahedravol(nt);
			}
		      else
			{
			  M(dofnti,dofntj)+=(q(nt)/20.0+(double)boostublas::prec_inner_prod(graddofnti,graddofntj))*tetrahedravol(nt);
			}
		    }
		}
	    }
