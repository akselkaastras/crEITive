...
int main(int argc,char *argv[])
{
  ...
  //Mesh filename to solve the Schr�dinger equation
  //or to interpolate the reconstructed conductivity
  //if the Calder�n approximation is chosen
  string meshfilename;
  ...
  // order N
  unsigned N = 0;
  // reciprocal max stop band magnitude
  unsigned A = 0;
  ...
  //Read commands
  read_commands(commandsfilename,ngrid,truncrad,iftmethodname,zetamethodname,pkappa,dnmapfilename,meshfilename,methodname,cgostracesfilename,qfilename,qhatfilename,conductivityfilename,iodnmap0,dnmap0filename,ios0,s0filename,N,A);
  ...
  read_dnmap(dnmapfilename,n,dnmap);
  ...
  if (methodname=="calderon"||methodname=="texp")
    {
      qhat_zeta_exp(dnmap,xn,wtnrep,xi,kappa,k,kT,matpsizeta,qhat,N,A);
    }
      ...
      else //Compute it
	{
      ...
      if (methodname=="t0")
	{
	  qhat_zeta_0(dnmap,S0,xn,wtnrep,xi,kappa,k,kT,matpsizeta,qhat,N,A);
	}
      else if (methodname=="t")
	{
	  qhat_zeta(dnmap,S0,xn,wtnrep,xi,kappa,k,kT,matpsizeta,qhat,N,A);
	}
    }
  ...
  if (methodname=="calderon")
	...
	{
	  //Calder�n formula with IFFT
	  ifft_calderon(xi,qhat);
	  // Real part
	  Array3Double q = realfunarray<3>(qhat);

	  //Trilinear interpolation
	  trilinear_interpolation(sig,nodes,q,-scale,scale,-scale,scale,-scale,scale,ngrid,ngrid,ngrid);
	}
      else if (iftmethodname=="ws") // Whittaker-Shannon method
	{
	  //Calder�n formula with Whittaker-Shannon interpolation
    ...
    // Test FT
    //double ximax = ((double)ngrid-1)*((double)ngrid-1)*Pi/((double)ngrid*2.0);
    //fft_q(ximax,qhat);
    //string s1 = "ifft";
    //string s2 = "qhat_dnmapid_13_true.dat";
    //save_qhat(qhat,xi,s2);
	  ...
	  //Trilinear interpolation
	  trilinear_interpolation(qbary,tetrahedrabary,q,-scale,scale,-scale,scale,-scale,scale,ngrid,ngrid,ngrid);
	}
      else if (iftmethodname=="ws") // Whittaker-Shannon method
	{
