...
void read_commands(string &commandsfilename,unsigned &ngrid, double &truncrad,string &iftmethodname,string &zetamethodname,double &pkappa,string &dnmapfilename,string &meshfilename,string &methodname,string &cgostracesfilename,string &qfilename,string &qhatfilename,string &conductivityfilename,string &iodnmap0,string &dnmap0filename,string &ios0,string &s0filename, unsigned &N, unsigned &A)
{
  ...
  if (methodname=="calderon")
    {
      std::cout << "Reconstruction with Calderon approximation" << std::endl;
    }
  if (methodname=="texp")
    {
  ...
  if (N>0)
	{
	  std::cout << "Truncation with Butterworth low-pass filter of order N=" << N <<" and A=" << A << std::endl;
	}
  if (N==0)
    {
