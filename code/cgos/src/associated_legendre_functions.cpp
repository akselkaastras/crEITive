...
void associated_legendre_functions(MatDouble &lf,const VecDouble &t)
{
  std::cout << "Computing fully normalized associated Legendre functions..." << std::endl;

  unsigned n=lf.size1();
  unsigned ndpone=lf.size2();
  ndpone=(intsqrt(1+8*ndpone)-1)/2;
  ...
  for (unsigned i=0;i<n;i++) //Values of t
    {
      ...
      for (unsigned k=2;k<ndpone;k++) //Degrees of Legendre functions
	{
